...
void clppSort_nvRadixSort::radixSortStepKeysOnly(unsigned int nbits, unsigned int startbit)
{
    ...
    scan.scanExclusiveLarge(mCountersSum, mCounters, 1, _datasetSize / 2 / CTA_SIZE*16);
...
void clppSort_nvRadixSort::radixSortBlocksKeysOnlyOCL(unsigned int nbits, unsigned int startbit)
{
    ...
    cl_int clStatus;
    clStatus  = clSetKernelArg(kernel_RadixSortBlocksKeysOnly, 0, sizeof(cl_mem), (void*)&_clBuffer_keys);
    ...
    clStatus |= clSetKernelArg(kernel_RadixSortBlocksKeysOnly, 4, sizeof(unsigned int), (void*)&_datasetSize);
...
void clppSort_nvRadixSort::findRadixOffsetsOCL(unsigned int startbit)
{
    ...
    clStatus |= clSetKernelArg(kernel_FindRadixOffsets, 4, sizeof(unsigned int), (void*)&_datasetSize);
...
void clppSort_nvRadixSort::reorderDataKeysOnlyOCL(unsigned int startbit)
{
    ...
    cl_int clStatus;
    clStatus  = clSetKernelArg(kernel_ReorderDataKeysOnly, 0, sizeof(cl_mem), (void*)&_clBuffer_keys);
	...
	clStatus |= clSetKernelArg(kernel_ReorderDataKeysOnly, 6, sizeof(unsigned int), (void*)&_datasetSize);
